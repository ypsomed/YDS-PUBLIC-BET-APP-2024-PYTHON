...
2. Zeige dem Spieler die Anzahl der Buchstaben im Wort als Unterstriche an (einer für jeden Buchstaben).
3. Der Spieler gibt einen Buchstaben ein.
4. Überprüfe, ob der Buchstabe im Wort vorkommt. Wenn ja, ersetze den entsprechenden Unterstrich durch den Buchstaben.
5. Wenn der Buchstabe nicht im Wort vorkommt, ziehe einen Versuch ab.
6. Wiederhole Schritte 3-5, bis das Wort vollständig geraten ist oder die Anzahl der Versuche aufgebraucht ist.
7. Wenn der Spieler das Wort vollständig erraten hat, gib eine Nachricht aus, die den Spieler beglückwünscht. Wenn der
   Spieler das Wort nicht erraten hat, gib eine Nachricht aus, die das Wort zeigt.
...
- Verwende `in` Operator, um zu überprüfen, ob der eingegebene Buchstabe im Wort vorkommt. Verwende `str.replace()`
  Methode, um das Wort zu aktualisieren.
- Verwende break Statement, um die Schleife zu beenden, wenn das Wort vollständig erraten wurde oder die Anzahl der
  Versuche aufgebraucht ist.
